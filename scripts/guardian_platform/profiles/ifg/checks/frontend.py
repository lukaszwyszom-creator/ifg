...
from guardian_platform.profiles.ifg.infra.git import git, parse_porcelain_line
from guardian_platform.profiles.ifg.infra.ssh import remote_git, ssh


def dist_js_blobs() -> str:
    dist_assets = REPO_ROOT / "frontend-react" / "dist" / "assets"
...
def check_ksef_connect_button_fix() -> tuple[bool, list[str]]:
    notes: list[str] = []
    ok = True
    ksef_js = REPO_ROOT / "frontend-react" / "src" / "api" / "ksef.js"
    tile = REPO_ROOT / "frontend-react" / "src" / "components" / "layout" / "KSeFConnectionTile.jsx"
