...
from guardian_platform.profiles.ifg.config.defaults import DEFAULT_REMOTE_PATH
from guardian_platform.profiles.ifg.doctor.aggregation import (
    aggregate_overall_status,
    stage_status_from_checks,
)
from guardian_platform.profiles.ifg.doctor.checks import (
    run_alembic_checks,
    run_backend_checks,
...
from guardian_platform.profiles.ifg.doctor.models import CheckResult, CheckStatus, DoctorState
from guardian_platform.profiles.ifg.doctor.report import render_json, render_markdown
from guardian_platform.profiles.ifg.doctor.service import add_checks, get_doctor_state
...
class RiskAggregationStage(_DoctorStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        ctx.transaction.profile_data["recommended_actions"] = [
...
class SummaryStage(_DoctorStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        ctx.transaction.profile_data["doctor"] = state.to_dict()
        ...
        if output_format in ("markdown", "terminal") or report_path:
            if output_format != "json" or report_path:
                ...
                ctx.transaction.profile_data.setdefault("artifacts", []).append(
                    {"type": "ifg_doctor_report", "path": str(out)}
                )
