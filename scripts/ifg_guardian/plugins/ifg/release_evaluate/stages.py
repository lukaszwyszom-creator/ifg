...
class DoctorDependencyStage(_EvaluateStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        warnings = [
            ...
            if c.status.value in {"WARN", "FAIL", "CRITICAL"}
        ]
        positives = [
            f"[{c.group}] {c.name}: {c.message}"
            for c in doctor.checks
...
class DeployCheckStage(_EvaluateStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        state.warnings.append("Deploy check returned non-zero; requires operator review.")
...
class TestDiscoveryStage(_EvaluateStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        msg = (proc.stderr or proc.stdout or "pytest collect failed").strip().splitlines()[-1]
        state.warnings.append(f"Test discovery warning: {msg}")
...
class DecisionStage(_EvaluateStage):
    ...
    def interpret(self, ctx: WorkflowContext, results: StageExecutionResults) -> StageResult:
        ...
        doctor = get_doctor_dependency(ctx)
        parts = score_from_doctor(doctor, test_discovery_ok=state.test_discovery_ok)
