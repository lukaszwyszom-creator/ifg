...
class LineEndingCategory(str, Enum):
    """Line-ending sub-classification for modified tracked files."""
...
def _run_git_bytes(args: list[str]) -> subprocess.CompletedProcess[bytes]:
    """Run git and return raw stdout (no EOL conversion)."""
    return subprocess.run(
        ["git", *args],
        cwd=ROOT,
        capture_output=True,
...
def analyze_line_endings(path: str) -> LineEndingAnalysis:
    """Classify whether a modified file differs only by line endings.

    Read-only: never runs git restore.
    """
    ...
    verification.append(
        f"{_checkmark(raw_head_worktree)} HEAD vs working tree (raw bytes)"
    )
    verification.append(
        f"{_checkmark(raw_head_index)} HEAD vs index (raw bytes)"
    )
    verification.append(
        f"{_checkmark(raw_index_worktree)} index vs working tree (raw bytes)"
    )

    diff_ignore_cr = _run_git(["diff", "--ignore-cr-at-eol", "--quiet", "--", path])
    ignore_cr_clean = diff_ignore_cr.returncode == 0
    verification.append(
        f"{_checkmark(ignore_cr_clean)} git diff --ignore-cr-at-eol"
    )
