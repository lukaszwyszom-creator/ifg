...
class TransmissionORM(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_transmissions_correlation_id_created_at", "correlation_id", "created_at"),
    ...
    correlation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)
    status: Mapped[str] = mapped_column(String(64), index=True)
    attempt_no: Mapped[int] = mapped_column(Integer, default=1)
    idempotency_key: Mapped[str | None] = mapped_column(String(255), nullable=True, index=False)
